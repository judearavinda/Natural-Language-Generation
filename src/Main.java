...
public class Main {
	...
	enum SearchType {bfs, dfs, heurisitic};

	static final String fileName = "input.txt";
	public static void main(String[] args) {
		print(sentenceGenerator("the", new String[]{"DT","NN","VBD","NNP","IN","DT","NN"}, stringfiedFile(fileName), SearchType.bfs));
		print(sentenceGenerator("the", new String[]{"DT","NN","VBD","NNP","IN","DT","NN"}, stringfiedFile(fileName), SearchType.dfs));
		print(sentenceGenerator("the", new String[]{"DT","NN","VBD","NNP","IN","DT","NN"}, stringfiedFile(fileName), SearchType.heurisitic));
...
//		print(sentenceGenerator("benjamin" , new String[]{"NNP","VBD","DT","JJS","NN"}, stringfiedFile(fileName)));
//		print(sentenceGenerator("a" , new String[]{"DT","NN","VBD","NNP","IN","DT","NN"}, stringfiedFile(fileName)));
	}
	...
	static String sentenceGenerator(String startingWord, String[] sentenceSpec, String graph, SearchType searchType){
		...
		if (searchType == SearchType.heurisitic){ //search for the highest probability first
		...
		while(currentNodes.size() > 0){
			switch (searchType) {
				...
				case dfs:
					...
					break;
				case heurisitic:
					currentTNode = currentNodes.pollLast(); //dfs - heuristic uses dfs
					break;
			}
			nodesConsidered++;
			if (currentTNode.numWords == sentenceSpec.length){	//Found a final node
			...
			for (Graph.Edge edge : childrenList) {
				//currentTNode.numWords is actually looks at the next index because starts its count at 1, not 0
